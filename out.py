...
#costruisco i grafi
for data in dates:
    ln = LN.LightningNetwork('datasetLun\ln_' + str(data)+ '.json__edges.json','datasetLun\ln_' + str(data)+ '.json__nodes.json')        #per analisi
    #ln = LN.LightningNetwork(channels_filename, nodes_filename, day, month=10, mode='graph_file_graphia', attributes=None)   #per graphia

    ln.crea_grafo("output\grafi_all\multigrafo_" + str(data) +  ".gml")            #per analisi
